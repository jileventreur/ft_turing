...
let generate file input =
	...
	let _transitions =
		...
		in List.map _buildAssoc ([
			...
			("MOVEBEGIN", function "~" -> ("~", "MOVE_RIGHT_1", "~", "RIGHT")
				| c -> (c, "MOVEBEGIN", c, "LEFT")) ;
			("WRITE_RIGHT", function "~" -> ("~", "HALT", "~", "RIGHT")
				| c -> (c, "WRITE_RIGHT_" ^ c, "_", "RIGHT")) ;
			("MOVESTART", function "~" -> ("~", "ERASE", "~", "RIGHT")
				| c -> (c, "MOVESTART", c, "LEFT")) ;
			("ERASE", function "|" -> ("|", "TEST", "~", "RIGHT")
				| c -> (c, "HALT", c, "RIGHT"))
			...
			function 7 -> ( "MOVE_RIGHT_7",
				function "|" -> ("|", "WRITE_RIGHT", "|", "RIGHT")
				| c -> (c, "MOVE_RIGHT_7", c, "RIGHT")
			)
